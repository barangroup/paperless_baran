...
function getGravatar (email) {
  if (email) {
    return "http://www.gravatar.com/avatar/"+md5(email)+"?d=mm&s=200";
  }else{
    return "http://www.gravatar.com/avatar/?d=mm&s=200";
  };
}
...
app.controller('MainController', ['$scope', '$http', '$location', '$timeout', function($scope, $http, $location, $timeout) {
  ...
  $scope.count = 0;
    ...
    // TODO has buggy in destroyer timer
    $scope.toast = new function() {

      this.startDestroyer = function(time) {
        $timeout(function() {
          $scope.alerts.shift();
        }, time);
      }
...
.controller("DashboardController", ['$scope', '$http', '$sce', function($scope, $http, $sce) {
    ...
    $scope.quote = {
      text : "میدونستی با خرید شارژ از <a target=\"_blank\" href=\"http://charge.barang.ir\">سایت کارت شارژ باران</a> بجز استفاده از خدمات شارژ غیر مستقیم و شارژ وایمکس میتونی تو شاد کردن بچه های ویژه هم ما رو همراهی کنی.",
      title : "سایت شارژ باران",
      author : null
    }
...
.controller("AddContactController", function($scope, $http, $window) {

...
app.controller('MainController', ['$scope', '$http', '$location', '$timeout', 'toaster', 'ngProgressFactory', 'ngProgress',
	...
	function($scope, $http, toaster) {
		...
		$scope.getData = function () {
			...
			$scope.isLoading = true;
			$http.get(s.sprintf('/remeberance?type=all&from=%d&to=%d', $scope.from, $scope.to))
		...
		$scope.likeThis = function (index) {
			$http.post('/remeberance', {type: 'like', data: {_remeberance_id: $scope.remeberances[index]._id}})
			.success(function (data) {
	...
	function($scope, $http, toaster) {
		...
		$scope.submitRemembrance = function () {
			$http.post('/remeberance', {type: 'add', data: $scope.remembrance})
			.success(function (data) {
				if (data.add){
...
app.filter('farsimal', function() {
	...
	return function (item) {
		...
		return s.sprintf("%s %s ماه %s", date[2], monthName[date[1]], date[0]);
	}
});
